...
void game_init(GameState* game, int width, int height, const GameConfig* cfg) {
    ...
    if(!game->food) { free(game->players); game->players = NULL; return; }
    for(int i = 0; i < game->max_players; i++) {
        ...
        if(!game->players[i].body) {
            ...
            free(game->food); free(game->players); game->players = NULL; game->food = NULL; return;
        }
    }
...
void game_free(GameState* game) {
    ...
    if(game->food) { free(game->food); game->food = NULL; }
}
