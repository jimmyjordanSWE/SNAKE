...
void game_step(Game* g, GameEvents* out_events)
{
    ...
    for (int i = 0;
         i < num_players && out_events->died_count < GAME_EVENTS_MAX_PLAYERS;
         i++)
    {
...
void game_init(GameState* game, int width, int height, const GameConfig* cfg)
{
    ...
    for (int i = 0; i < game->max_players; i++)
    {
        ...
        game->players[i].prev_segment_x =
            (float*)malloc(sizeof(float) * (size_t)game->max_length);
        game->players[i].prev_segment_y =
            (float*)malloc(sizeof(float) * (size_t)game->max_length);
