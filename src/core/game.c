...
int game_enqueue_input(Game* g, int player_index, const InputState* in) {
    ...
    if (in->pause_toggle)
        g->state.status = (g->state.status == GAME_STATUS_PAUSED) ? GAME_STATUS_RUNNING : GAME_STATUS_PAUSED;
    if (in->restart)
...
void game_init(GameState* game, int width, int height, const GameConfig* cfg) {
    ...
    if ((size_t)game->max_players > SIZE_MAX / sizeof(PlayerState))
        return;
    if ((size_t)game->max_food > SIZE_MAX / sizeof(SnakePoint))
        return;
