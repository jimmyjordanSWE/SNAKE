...
s->players[idx].color= color;
s->players[idx].score= 0;
s->players[idx].died_this_tick= false;
s->players[idx].score_at_death= 0;
s->players[idx].active= true;
s->players[idx].needs_reset= false;
s->players[idx].length= 0;
s->players[idx].is_remote= true;
return idx;
}
void game_set_food_sync_only(Game* g, bool enable) {
if(g) {
g->state.food_sync_only= enable;
if(enable) g->state.food_count= 0;
}
}
...
if(game == NULL || game->food_sync_only) return;
...
for(int i= 0; i < player->length; ++i) {
player->prev_segment[i].x= (float)player->body[i].x + 0.5f;
player->prev_segment[i].y= (float)player->body[i].y + 0.5f;
}
bool actual_grow= grow && player->length < SNAKE_BODY_MAX_LEN;
...
if(actual_grow) {
player->prev_segment[last].x= (float)player->body[last].x + 0.5f;
player->prev_segment[last].y= (float)player->body[last].y + 0.5f;
player->length++;
}
}
...
player->body[0]= head;
player->body[1]= tail;
player->prev_head.x= (float)head.x + 0.5f;
player->prev_head.y= (float)head.y + 0.5f;
player->prev_segment[0].x= (float)player->body[0].x + 0.5f;
player->prev_segment[0].y= (float)player->body[0].y + 0.5f;
player->prev_segment[1].x= (float)player->body[1].x + 0.5f;
player->prev_segment[1].y= (float)player->body[1].y + 0.5f;
for(int k= 2; k < SNAKE_BODY_MAX_LEN; ++k) {
player->prev_segment[k].x= 0.0f;
player->prev_segment[k].y= 0.0f;
}
return true;
}
}
...
if(i == 0)
name= game_config_get_player_name(cfg);
else
name= game_config_get_player_name_for(cfg, i);
if(name && name[0]) {
strncpy(game->players[i].name, name, PERSIST_PLAYER_NAME_MAX - 1);
game->players[i].name[PERSIST_PLAYER_NAME_MAX - 1]= '\0';
} else {
snprintf(game->players[i].name, PERSIST_PLAYER_NAME_MAX, "Player %d", i + 1);
}
}
food_respawn(game);
}
...
game->players[i].lives= (game->num_players > 1) ? 3 : 0;
game->players[i].eliminated= false;
game->players[i].prev_head.x= 0.0f;
game->players[i].prev_head.y= 0.0f;
}
...
/* create skip for remote players: they only update via network state */
if(player->is_remote) continue;
...
for(int i= 0; i < num_players; i++) {
PlayerState* player= &game->players[i];
/* Skip death processing for remote players (handled by state sync) */
if(player->is_remote) continue;
...
for(int i= 0; i < num_players; i++) {
PlayerState* player= &game->players[i];
/* Skip respawn for remote players */
if(player->is_remote) continue;
...
/* Multiplayer: end when <= 1 active players remain (last man standing).
                                                                                   Single-player: end when no active
           players remain. */
if(game->num_players > 1) {
/* Check if we are still waiting for players or if the game is truly over.
             * In a networked game, we might wait for the server to decide game over.
             * For now, simplistic check: if only 1 local player + remote players remain, keep going.
             * Actually, just checking active count is probably fine as long as remote players stay active.
             */
/* For remote play, we don't want local logic to end the game prematurely if just one person is connected.
             */
/* But if active_count <= 1, it means everyone else died. */
if(active_count <= 1) {
/* If we have remote players, we might rely on them telling us they are dead (inactive). */
/* So this logic is probably okay: if everyone is dead or inactive, game over. */
...
PlayerState* player= &game->players[i];
if(!player->active || player->length <= 0) continue;
/* Remote players: skip local movement and eating */
if(player->is_remote) continue;
if(player->needs_reset) continue;
SnakePoint current_head= player->body[0];
player->prev_head.x= (float)current_head.x + 0.5f;
player->prev_head.y= (float)current_head.y + 0.5f;
