 ...
 * Functions are intentionally conservative and return 0/false on malformed input.
 */
...
bool net_unpack_input(const unsigned char* buf, size_t buf_size, InputState* out) {
	if(!buf || buf_size < 1 || !out) return false;
...
size_t net_pack_game_state(const GameState* game, unsigned char* buf, size_t buf_size) {
	...
	unsigned char* p = buf;
	uint32_t v;
	v = htonl((uint32_t)game->width); memcpy(p, &v, 4); p += 4;
	v = htonl((uint32_t)game->height); memcpy(p, &v, 4); p += 4;
	v = htonl(game->rng_state); memcpy(p, &v, 4); p += 4;
	v = htonl((uint32_t)game->status); memcpy(p, &v, 4); p += 4;
	v = htonl((uint32_t)game->num_players); memcpy(p, &v, 4); p += 4;
	v = htonl((uint32_t)game->food_count); memcpy(p, &v, 4); p += 4;
	for(int i = 0; i < game->food_count; i++) {
		v = htonl((uint32_t)game->food[i].x); memcpy(p, &v, 4); p += 4;
		v = htonl((uint32_t)game->food[i].y); memcpy(p, &v, 4); p += 4;
	}
	for(int i = 0; i < game->num_players; i++) {
		v = htonl((uint32_t)game->players[i].score); memcpy(p, &v, 4); p += 4;
		v = htonl((uint32_t)game->players[i].length); memcpy(p, &v, 4); p += 4;
	}
	return needed;
}

bool net_unpack_game_state(const unsigned char* buf, size_t buf_size, GameState* out) {
	if(!buf || !out || buf_size < 24) return false;
	const unsigned char* p = buf;
	uint32_t v;
	memcpy(&v, p, 4); out->width = (int)ntohl(v); p += 4;
	memcpy(&v, p, 4); out->height = (int)ntohl(v); p += 4;
	memcpy(&v, p, 4); out->rng_state = ntohl(v); p += 4;
	memcpy(&v, p, 4); out->status = (GameStatus)ntohl(v); p += 4;
	memcpy(&v, p, 4); out->num_players = (int)ntohl(v); p += 4;
	...
	for(int i = 0; i < out->num_players; i++) {
		memcpy(&v, p, 4); out->players[i].score = (int)ntohl(v); p += 4;
		memcpy(&v, p, 4); out->players[i].length = (int)ntohl(v); p += 4;
	}
	return true;
}
struct NetClient {
