...
c->player_color_arr[i]= 0xFF000000 | (0x00330000 * (unsigned int)i) | (0x00003300 * (unsigned int)i) | (0x00000033 * (unsigned int)i);
}
...
if(!cfg) return;
if(w_out) *w_out= cfg->board_width;
if(h_out) *h_out= cfg->board_height;
}
...
if(!cfg) return;
if(w_out) *w_out= cfg->screen_width;
if(h_out) *h_out= cfg->screen_height;
}
...
if(cfg) cfg->wall_texture_scale= v;
}
float game_config_get_wall_texture_scale(const GameConfig* cfg) { return cfg ? cfg->wall_texture_scale : 1.0f; }
void game_config_set_floor_texture_scale(GameConfig* cfg, float v) {
if(cfg) cfg->floor_texture_scale= v;
}
float game_config_get_floor_texture_scale(const GameConfig* cfg) { return cfg ? cfg->floor_texture_scale : 1.0f; }
...
if(!cfg || player_idx < 0 || player_idx >= SNAKE_MAX_PLAYERS) return;
cfg->key_left_arr[player_idx]= c;
if(player_idx == 0) cfg->key_left= c;
}
char game_config_get_player_key_left(const GameConfig* cfg, int player_idx) {
if(!cfg || player_idx < 0 || player_idx >= SNAKE_MAX_PLAYERS) return '\0';
...
if(player_idx == 0) cfg->key_right= c;
}
...
if(!cfg || !name || player_idx < 0 || player_idx >= cfg->max_players) return;
snprintf(cfg->player_name_arr[player_idx], PERSIST_PLAYER_NAME_MAX, "%s", name);
if(player_idx == 0) snprintf(cfg->player_name, PERSIST_PLAYER_NAME_MAX, "%s", cfg->player_name_arr[0]);
}
const char* game_config_get_player_name_for(const GameConfig* cfg, int player_idx) {
if(!cfg || player_idx < 0 || player_idx >= cfg->max_players) return "";
...
if(!cfg || player_idx < 0 || player_idx >= cfg->max_players) return 0u;
...
int game_config_get_mp_enabled(const GameConfig* cfg) { return cfg ? cfg->mp_enabled : 0; }
...
const char* game_config_get_mp_server_host(const GameConfig* cfg) { return cfg ? cfg->mp_server_host : NULL; }
int game_config_get_mp_server_port(const GameConfig* cfg) { return cfg ? cfg->mp_server_port : 0; }
void game_config_set_mp_identifier(GameConfig* cfg, const char* id) {
if(!cfg || !id) return;
snprintf(cfg->mp_identifier, PERSIST_MP_IDENTIFIER_MAX, "%s", id);
}
const char* game_config_get_mp_identifier(const GameConfig* cfg) { return cfg ? cfg->mp_identifier : NULL; }
...
const char* game_config_get_mp_session(const GameConfig* cfg) { return cfg ? cfg->mp_session : NULL; }
int game_config_get_mp_is_host(const GameConfig* cfg) { return cfg ? cfg->mp_is_host : 1; }
void game_config_set_headless(GameConfig* cfg, int v) {
if(!cfg) return;
cfg->headless= v ? 1 : 0;
}
int game_config_get_headless(const GameConfig* cfg) { return cfg ? cfg->headless : 0; }
void game_config_set_autoplay(GameConfig* cfg, int v) {
if(!cfg) return;
cfg->autoplay= v ? 1 : 0;
}
int game_config_get_autoplay(const GameConfig* cfg) { return cfg ? cfg->autoplay : 0; }
...
while(*p && isspace((unsigned char)*p)) p++;
if(*p == '\0') continue; /* no score */
char* score_str= trim_in_place(p);
if(score_str == NULL || *score_str == '\0') continue;
char* endptr= NULL;
errno= 0;
...
if(errno == 0 && endptr != value) config->render_glyphs= clamp_int((int)v, 0, 1);
} else {
...
if(errno == 0 && endptr != value) config->fov_degrees= (float)dv;
} else if(strcmp(key, "wall_height_scale") == 0) {
char* endptr= NULL;
errno= 0;
double dv= strtod(value, &endptr);
if(errno == 0 && endptr != value) config->wall_height_scale= (float)dv;
} else if(strcmp(key, "tail_height_scale") == 0) {
char* endptr= NULL;
errno= 0;
double dv= strtod(value, &endptr);
if(errno == 0 && endptr != value) config->tail_height_scale= (float)dv;
} else if(strcmp(key, "wall_texture_scale") == 0) {
char* endptr= NULL;
errno= 0;
double dv= strtod(value, &endptr);
if(errno == 0 && endptr != value) config->wall_texture_scale= (float)dv;
} else if(strcmp(key, "floor_texture_scale") == 0) {
char* endptr= NULL;
errno= 0;
double dv= strtod(value, &endptr);
if(errno == 0 && endptr != value) config->floor_texture_scale= (float)dv;
...
if(config->max_players < config->num_players) config->max_players= config->num_players;
...
} else if(strcmp(key, "mp_is_host") == 0) {
for(char* p= value; *p; p++) *p= (char)tolower((unsigned char)*p);
if(strcmp(value, "true") == 0 || strcmp(value, "yes") == 0 || strcmp(value, "1") == 0)
config->mp_is_host= 1;
else if(strcmp(value, "false") == 0 || strcmp(value, "no") == 0 || strcmp(value, "0") == 0)
...
if(fprintf(fp, "wall_texture_scale=%.2f\n", config->wall_texture_scale) < 0) goto write_fail;
if(fprintf(fp, "floor_texture_scale=%.2f\n", config->floor_texture_scale) < 0) goto write_fail;
...
if(fprintf(fp, "mp_enabled=%s\n", (config->mp_enabled ? "true" : "false")) < 0) goto write_fail;
if(fprintf(fp, "mp_server_host=%s\n", config->mp_server_host) < 0) goto write_fail;
if(fprintf(fp, "mp_server_port=%d\n", config->mp_server_port) < 0) goto write_fail;
if(fprintf(fp, "mp_identifier=%s\n", config->mp_identifier) < 0) goto write_fail;
if(fprintf(fp, "mp_session=%s\n", config->mp_session) < 0) goto write_fail;
if(fprintf(fp, "mp_is_host=%s\n", (config->mp_is_host ? "true" : "false")) < 0) goto write_fail;
...
if(fprintf(fp, "p%d_left=%c\n", p + 1, config->key_left_arr[p]) < 0) goto write_fail;
if(fprintf(fp, "p%d_right=%c\n", p + 1, config->key_right_arr[p]) < 0) goto write_fail; /* per-player name and color */
if(fprintf(fp, "p%d_name=%s\n", p + 1, config->player_name_arr[p]) < 0) goto write_fail;
if(fprintf(fp, "p%d_color=0x%08x\n", p + 1, (unsigned int)config->player_color_arr[p]) < 0) goto write_fail;
}
...
if(strcmp(key, "wall_texture_scale") == 0) return true;
if(strcmp(key, "floor_texture_scale") == 0) return true;
...
size_t bl= strlen(b);
if(strcmp(key, b) == 0) return true;
if(strncmp(key, b, bl) == 0 && key[bl] == '_') {
char* endptr= NULL;
errno= 0;
long v= strtol(key + bl + 1, &endptr, 10);
if(errno == 0 && endptr != key + bl + 1 && *endptr == '\0' && v >= 1 && v <= SNAKE_MAX_PLAYERS) return true;
}
}
...
if(endptr != key + 1 && *endptr == '_') {
const char* suffix= endptr + 1;
if((strcmp(suffix, "left") == 0 || strcmp(suffix, "right") == 0 || strcmp(suffix, "name") == 0 || strcmp(suffix, "color") == 0) && v >= 1 && v <= SNAKE_MAX_PLAYERS) return true;
}
}
if(strcmp(key, "key_quit") == 0 || strcmp(key, "key_restart") == 0 || strcmp(key, "key_pause") == 0) return true;
...
if(strcmp(key, "headless") == 0 || strcmp(key, "autoplay") == 0) return true;
return false;
}
