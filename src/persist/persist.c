...
{
    if (filename == NULL || scores == NULL || max_count <= 0)
        return 0;
...
bool persist_write_scores(const char*      filename,
...
bool persist_load_config(const char* filename, GameConfig* config)
{
    ...
    config->enable_external_3d_view =
        PERSIST_CONFIG_DEFAULT_ENABLE_EXTERNAL_3D_VIEW;
    /* new defaults */
