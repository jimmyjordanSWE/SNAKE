...
GameConfig* game_config_create(void)
{
    ...
    snprintf(c->wall_texture,
             PERSIST_TEXTURE_PATH_MAX,
             "%s",
             PERSIST_CONFIG_DEFAULT_WALL_TEXTURE);
    snprintf(c->floor_texture,
             PERSIST_TEXTURE_PATH_MAX,
             "%s",
...
int persist_read_scores(const char* filename, HighScore*** out_scores)
{
    ...
    HighScore** arr =
        (HighScore**)calloc((size_t)PERSIST_MAX_SCORES, sizeof(HighScore*));
    int  count = 0;
    char buffer[PERSIST_SCORE_BUFFER];
    while (count < PERSIST_MAX_SCORES
           && fgets(buffer, (int)sizeof(buffer), fp) != NULL)
    {
        ...
        if (errno != 0 || endptr == score_str || score_long < 0
            || score_long > INT_MAX)
            continue;
...
{
    if (filename == NULL || scores == NULL || count < 0
        || count > PERSIST_MAX_SCORES)
        return false;
...
bool persist_append_score(const char* filename, const char* name, int score)
{
    ...
    {
        arr =
            (HighScore**)realloc(arr, (size_t)(count + 1) * sizeof(HighScore*));
        if (!arr)
            return false;
    ...
    while (fgets(buffer, (int)sizeof(buffer), fp) != NULL)
    {
        ...
        *eq_pos      = '\0';
        ...
        if (key == NULL || value == NULL || *key == '\0' || *value == '\0')
            continue;
        if (strcmp(key, "render_glyphs") == 0 || strcmp(key, "glyphs") == 0
            || strcmp(key, "charset") == 0)
        {
            ...
            else
            {
                ...
                if (strcmp(value, "utf8") == 0 || strcmp(value, "unicode") == 0
                    || strcmp(value, "box") == 0)
        ...
        {
            snprintf(
                config->wall_texture, PERSIST_TEXTURE_PATH_MAX, "%s", value);
            continue;
        }
        else if (strcmp(key, "floor_texture") == 0)
        {
            snprintf(
                config->floor_texture, PERSIST_TEXTURE_PATH_MAX, "%s", value);
            continue;
        }
        else if (strcmp(key, "key_up") == 0 || strcmp(key, "key_down") == 0
                 || strcmp(key, "key_left") == 0
                 || strcmp(key, "key_right") == 0
                 || strcmp(key, "key_quit") == 0
                 || strcmp(key, "key_restart") == 0
...
bool persist_write_config(const char* filename, const GameConfig* config)
{
    ...
    if (fprintf(fp,
                "render_glyphs=%s\n",
                (config->render_glyphs == 1) ? "ascii" : "utf8")
        < 0)
        goto write_fail;
    ...
    if (fprintf(fp, "screen_height=%d\n", config->screen_height) < 0)
        goto write_fail;
    if (fprintf(fp,
                "enable_external_3d_view=%s\n",
                (config->enable_external_3d_view ? "true" : "false"))
        < 0)
        goto write_fail;
    ...
    if (fprintf(fp,
                "show_sprite_debug=%s\n",
                config->show_sprite_debug ? "true" : "false")
        < 0)
        goto write_fail;
...
static bool is_known_config_key(const char* key)
{
    ...
    if (strcmp(key, "floor_texture") == 0)
        return true;
    if (strcmp(key, "key_up") == 0 || strcmp(key, "key_down") == 0
        || strcmp(key, "key_left") == 0 || strcmp(key, "key_right") == 0
        || strcmp(key, "key_quit") == 0 || strcmp(key, "key_restart") == 0
        || strcmp(key, "key_pause") == 0)
        return true;
