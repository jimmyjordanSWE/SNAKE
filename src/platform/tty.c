...
if(tcsetattr(ctx->tty_fd, TCSADRAIN, &ctx->orig_termios) == -1) { perror("tty_open: tcsetattr cleanup (overflow)"); }
(void)close(ctx->tty_fd);
free(ctx);
return NULL;
}
if(n_cells > SIZE_MAX / sizeof(struct ascii_pixel)) {
if(tcsetattr(ctx->tty_fd, TCSADRAIN, &ctx->orig_termios) == -1) { perror("tty_open: tcsetattr cleanup (n_cells)"); }
(void)close(ctx->tty_fd);
free(ctx);
return NULL;
}
...
if(!ctx->front || !ctx->back) { goto error; }
if(n_cells > SIZE_MAX / 32) {
error:
free(ctx->front);
free(ctx->back);
if(tcsetattr(ctx->tty_fd, TCSADRAIN, &ctx->orig_termios) == -1) { perror("tty_open: tcsetattr cleanup (error)"); }
(void)close(ctx->tty_fd);
free(ctx);
return NULL;
}
ctx->write_buffer_size= n_cells * 32;
ctx->write_buffer= malloc(ctx->write_buffer_size);
if(!ctx->front || !ctx->back || !ctx->write_buffer) {
free(ctx->front);
free(ctx->back);
free(ctx->write_buffer);
if(tcsetattr(ctx->tty_fd, TCSADRAIN, &ctx->orig_termios) == -1) { perror("tty_open: tcsetattr cleanup (buffers)"); }
(void)close(ctx->tty_fd);
free(ctx);
return NULL;
}
