...
static const char* const ANSI_FG[16]= {"\x1b[30m", "\x1b[31m", "\x1b[32m", "\x1b[33m", "\x1b[34m", "\x1b[35m", "\x1b[36m", "\x1b[37m", "\x1b[90m", "\x1b[91m", "\x1b[92m", "\x1b[93m", "\x1b[94m", "\x1b[95m", "\x1b[96m", "\x1b[97m"};
static const char* const ANSI_BG[16]= {"\x1b[40m", "\x1b[41m", "\x1b[42m", "\x1b[43m", "\x1b[44m", "\x1b[45m", "\x1b[46m", "\x1b[47m", "\x1b[100m", "\x1b[101m", "\x1b[102m", "\x1b[103m", "\x1b[104m", "\x1b[105m", "\x1b[106m", "\x1b[107m"};
...
if(proposed_write_buffer_size > TTY_WRITE_BUFFER_CAP) proposed_write_buffer_size= TTY_WRITE_BUFFER_CAP;
...
if(signal(SIGWINCH, sigwinch_handler) == SIG_ERR) { perror("signal(SIGWINCH) in tty"); }
ctx->dirty= true;
return ctx;
out:
if(ctx) {
...
if(have_orig) {
if(tcsetattr(ctx->tty_fd, TCSADRAIN, &ctx->orig_termios) == -1) { perror("tty_open: tcsetattr cleanup"); }
}
if(ctx->tty_fd != -1) (void)close(ctx->tty_fd);
free(ctx);
}
(void)err;
return NULL;
...
if(tcsetattr(ctx->tty_fd, TCSADRAIN, &ctx->orig_termios) == -1) { perror("tty_close: tcsetattr"); }
free(ctx->front);
free(ctx->back);
free(ctx->write_buffer);
close(ctx->tty_fd);
...
if(buf_safe_append_char(buf, &pos, &remaining, '\x1b') < 0) return;
if(buf_safe_append_char(buf, &pos, &remaining, '[') < 0) return;
if(buf_append_uint(buf, &pos, &remaining, (unsigned int)(y + 1)) < 0) return;
if(buf_safe_append_char(buf, &pos, &remaining, ';') < 0) return;
if(buf_append_uint(buf, &pos, &remaining, (unsigned int)(span_start + 1)) < 0) return;
if(buf_safe_append_char(buf, &pos, &remaining, 'H') < 0) return;
...
bool tty_calc_resize_requirements(long new_width, long new_height, size_t* out_cells, size_t* out_pixel_bytes, size_t* out_write_buffer_size) {
if(new_width <= 0 || new_height <= 0) return false;
size_t w= (size_t)new_width;
size_t h= (size_t)new_height;
/* check overflow for w * h */
if(h > SIZE_MAX / w) return false;
size_t cells= w * h;
if(cells == 0) return false;
if(cells > SIZE_MAX / sizeof(struct ascii_pixel)) return false;
size_t pixel_bytes= cells * sizeof(struct ascii_pixel);
/* check write buffer size overflow and cap */
if(cells > SIZE_MAX / 32) return false;
size_t write_sz= cells * 32;
if(write_sz > TTY_WRITE_BUFFER_CAP) write_sz= TTY_WRITE_BUFFER_CAP;
if(out_cells) *out_cells= cells;
if(out_pixel_bytes) *out_pixel_bytes= pixel_bytes;
if(out_write_buffer_size) *out_write_buffer_size= write_sz;
return true;
}
...
if(!tty_calc_resize_requirements((long)new_width, (long)new_height, &new_cells, &new_pixel_bytes, &new_write_buffer_size)) return false;
...
if(on_resize_cb) on_resize_cb(ctx, old_width, old_height, new_width, new_height, ctx->callback_userdata);
if(!ctx->size_valid && old_size_valid) {
void (*on_size_invalid_cb)(tty_context*, int, int, int, int, void*)= ctx->on_size_invalid;
if(on_size_invalid_cb) on_size_invalid_cb(ctx, new_width, new_height, ctx->min_width, ctx->min_height, ctx->callback_userdata);
}
ctx->dirty= true;
return true;
}
