...
tty_context* tty_open(const char* tty_path, int min_width, int min_height)
{
    ...
    if ((min_width > 0 && actual_width < min_width)
        || (min_height > 0 && actual_height < min_height))
    ...
    size_t buffer_size = (size_t)actual_width * (size_t)actual_height
                         * sizeof(struct ascii_pixel);
    ...
    struct ascii_pixel default_pixel =
        PIXEL_MAKE(' ', COLOR_DEFAULT_FG, COLOR_DEFAULT_BG);
...
void tty_flip(tty_context* ctx)
{
    ...
    for (int y = 0; y < ctx->height; y++)
    {
        ...
        {
            if (ascii_pixel_equal(ctx->front[y * ctx->width + x],
                                  ctx->back[y * ctx->width + x]))
                continue;
            int      span_start = x;
            uint16_t span_color = ctx->back[y * ctx->width + x].color;
            while (x < ctx->width
                   && !ascii_pixel_equal(ctx->front[y * ctx->width + x],
                                         ctx->back[y * ctx->width + x])
                   && ctx->back[y * ctx->width + x].color == span_color)
                x++;
            int span_end = x;
            int n        = snprintf(
                buf + pos, remaining, "\x1b[%d;%dH", y + 1, span_start + 1);
            ...
            if (fg != (uint8_t)current_fg || bg != (uint8_t)current_bg)
            {
                ...
                if (fg < 8)
                    n = snprintf(
                        buf + pos, remaining, "\x1b[3%um", (unsigned int)fg);
                else
                    n = snprintf(buf + pos,
                                 remaining,
                                 "\x1b[9%um",
                ...
                if (bg < 8)
                    n = snprintf(
                        buf + pos, remaining, "\x1b[4%um", (unsigned int)bg);
                else
                    n = snprintf(buf + pos,
                                 remaining,
                                 "\x1b[10%um",
...
bool tty_check_resize(tty_context* ctx)
{
    ...
    size_t new_size =
        (size_t)new_width * (size_t)new_height * sizeof(struct ascii_pixel);
    ...
    if ((ctx->min_width > 0 && new_width < ctx->min_width)
        || (ctx->min_height > 0 && new_height < ctx->min_height))
        ctx->size_valid = false;
    else
        ctx->size_valid = true;
    if (ctx->on_resize)
...
void tty_get_board_min_size(int  board_width,
                            int  board_height,
                            int* min_width,
                            int* min_height)
{
...
bool tty_size_sufficient_for_board(int term_width,
                                   int term_height,
                                   int board_width,
                                   int board_height)
{
