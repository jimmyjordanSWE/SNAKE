...
{
    if (!camera || camera->update_interval <= 0.0f)
        return;
...
{
    return cam ? cam->fov_radians : 0.0f;
}
...
{
    return cam ? cam->update_interval : 0.0f;
}
...
{
    return cam ? cam->interp_time : 0.0f;
}
...
void camera_camera_to_world(const Camera3D* camera,
                            ...
                            float*          world_y_out)
{
    if (!camera || !world_x_out || !world_y_out)
        return;
