...
/* Helper: Compute shortest angular difference (handles wraparound) */
...
/* Helper: Interpolate between two angles (shortest path) */
static float interpolate_angle(float from, float to, float t) {
    float diff = angle_difference(from, to);
    return from + diff * t;
}

/* Helper: Update precomputed vectors from angle */
...
void camera_set_from_player(Camera3D* camera, int x, int y, int dir) {
    ...
    float angle;
    switch (dir) {
    case 0: /* SNAKE_DIR_RIGHT */
        angle = 0.0f;
        break;
    case 1: /* SNAKE_DIR_DOWN */
        angle = (float)M_PI * 0.5f;
        break;
    case 2: /* SNAKE_DIR_LEFT */
        angle = (float)M_PI;
        break;
    case 3: /* SNAKE_DIR_UP */
        angle = (float)M_PI * 1.5f;
        break;
    default:
        angle = 0.0f;
        break;
    ...
    /* Reset interpolation to start of new frame */
    camera->interp_time = 0.0f;

    /* Update precomputed vectors */
    update_vectors(camera);
}
...
void camera_update_interpolation(Camera3D* camera, float delta_time) {
    ...
    /* Clamp to [0, update_interval] */
    if (camera->interp_time > camera->update_interval) {
...
void camera_set_interpolation_time(Camera3D* camera, float time) {
    ...
    if (time < 0.0f) camera->interp_time = 0.0f;
    else if (time > camera->update_interval) camera->interp_time = camera->update_interval;
    else camera->interp_time = time;
}

void camera_get_ray_angle(const Camera3D* camera, int col, float* ray_angle_out) {
    if (!camera || !ray_angle_out) return;

    /* Compute normalized column position [-0.5, 0.5] */
    float col_normalized = ((float)col - (float)camera->screen_width / 2.0f) / (float)camera->screen_width;

    /* Angle offset from camera center based on FOV */
...
void camera_world_to_camera(const Camera3D* camera, float world_x, float world_y,
                            float* cam_x_out, float* cam_y_out) {
    ...
    /* Rotate by -camera_angle to align with camera axes */
...
void camera_camera_to_world(const Camera3D* camera, float cam_x, float cam_y,
                            float* world_x_out, float* world_y_out) {
...
float camera_get_interpolated_angle(const Camera3D* camera) {
    ...
    /* Spherical linear interpolation between angles (handles wraparound) */
