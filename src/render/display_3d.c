...
void display_3d_init(Display3D* disp, int width, int height)
{
    ...
    disp->width  = width;
    disp->height = height;
    disp->buffer = (Framebuffer3DCell*)malloc(sizeof(Framebuffer3DCell)
                                              * (size_t)width * (size_t)height);
    if (disp->buffer)
        display_3d_clear(disp, ' ', 0);
}
...
void display_3d_draw_column(Display3D* disp,
                            int        col,
                            int        start_row,
                            int        end_row,
                            char       ch,
                            uint16_t   color)
{
    if (!disp || col < 0 || col >= disp->width)
        return;
    if (start_row < 0)
        start_row = 0;
    if (end_row >= disp->height)
        end_row = disp->height - 1;
    for (int row = start_row; row <= end_row; row++)
        display_3d_draw_cell(disp, col, row, ch, color);
}
void display_3d_draw_cell(
    Display3D* disp, int x, int y, char ch, uint16_t color)
{
    if (!disp || !disp->buffer || x < 0 || x >= disp->width || y < 0
        || y >= disp->height)
        return;
...
void display_3d_draw_row(Display3D* disp,
                         int        row,
                         int        start_col,
                         int        end_col,
                         char       ch,
                         uint16_t   color)
{
    if (!disp || row < 0 || row >= disp->height)
        return;
    if (start_col < 0)
        start_col = 0;
    if (end_col >= disp->width)
        end_col = disp->width - 1;
    for (int col = start_col; col <= end_col; col++)
        display_3d_draw_cell(disp, col, row, ch, color);
}
const Framebuffer3DCell*
display_3d_get_cell(const Display3D* disp, int x, int y)
{
    if (!disp || !disp->buffer || x < 0 || x >= disp->width || y < 0
        || y >= disp->height)
        return NULL;
