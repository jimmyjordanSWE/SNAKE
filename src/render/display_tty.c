...
/* DisplayContext wraps the tty_context */
struct DisplayContext {
    tty_context* tty;
};

/* Map display colors to TTY colors (they're the same, but this maintains abstraction) */
...
void display_clear(DisplayContext* ctx) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) { tty_put_pixel(ctx->tty, x, y, black); }
    }
}
...
void display_put_string(DisplayContext* ctx, int x, int y, const char* str, uint16_t fg, uint16_t bg) {
    if (!ctx || !ctx->tty || !str) { return; }

    int width = 0, height = 0;
    tty_get_size(ctx->tty, &width, &height);

    for (int i = 0; str[i] && x + i < width && y >= 0 && y < height; i++) { display_put_char(ctx, x + i, y, (uint16_t)str[i], fg, bg); }
}

void display_put_hline(DisplayContext* ctx, int x, int y, int len, uint16_t ch, uint16_t fg, uint16_t bg) {
    if (!ctx || !ctx->tty || len <= 0) { return; }

    int width = 0, height = 0;
    tty_get_size(ctx->tty, &width, &height);

    for (int i = 0; i < len && x + i < width; i++) { display_put_char(ctx, x + i, y, ch, fg, bg); }
}
...
void display_put_vline(DisplayContext* ctx, int x, int y, int len, uint16_t ch, uint16_t fg, uint16_t bg) {
    ...
    for (int i = 0; i < len && y + i < height; i++) { display_put_char(ctx, x, y + i, ch, fg, bg); }
}
