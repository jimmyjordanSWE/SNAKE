...
/* Palette for multiple players: Yellow, Red, Magenta, Cyan, Blue, Green */
static const uint16_t color_palette[]= {DISPLAY_COLOR_BRIGHT_YELLOW, DISPLAY_COLOR_BRIGHT_RED, DISPLAY_COLOR_BRIGHT_MAGENTA, DISPLAY_COLOR_BRIGHT_CYAN, DISPLAY_COLOR_BRIGHT_BLUE, DISPLAY_COLOR_BRIGHT_GREEN};
uint16_t snake_color= color_palette[p % (int)(sizeof(color_palette) / sizeof(color_palette[0]))];
...
char score_str[64];
const char* name= game->players[p].name;
if(name && name[0])
snprintf(score_str, sizeof(score_str), "%s%s: %d", game->players[p].is_remote ? "[MP] " : "", name, game->players[p].score);
else
snprintf(score_str, sizeof(score_str), "%sP%d: %d", game->players[p].is_remote ? "[MP] " : "", p + 1, game->players[p].score);
static const uint16_t color_palette[]= {DISPLAY_COLOR_BRIGHT_YELLOW, DISPLAY_COLOR_BRIGHT_RED, DISPLAY_COLOR_BRIGHT_MAGENTA, DISPLAY_COLOR_BRIGHT_CYAN, DISPLAY_COLOR_BRIGHT_BLUE, DISPLAY_COLOR_BRIGHT_GREEN};
uint16_t color= color_palette[p % (int)(sizeof(color_palette) / sizeof(color_palette[0]))];
...
for(int i= 0; i < g_mp_msg_count && (mp_y + i) < display_height; ++i) { draw_string(1, mp_y + i, g_mp_messages[i], DISPLAY_COLOR_CYAN); }
...
if(max_len > 0 && player_name_out[0] == '\0') snprintf(player_name_out, (size_t)max_len, "You");
}
...
/* Use a sanitizer helper so we can unit-test trimming and bounds logic */
#include "render_input.h"
char sanitized[16];
(void)render_sanitize_player_name(name_input, sanitized, sizeof(sanitized));
/* copy into caller buffer safely */
if(max_len <= 0) {
/* nothing to do */
return;
}
/* cap is space for characters (leave room for NUL) */
size_t cap= (size_t)max_len - 1;
if(sanitized[0] != '\0') {
size_t n= strnlen(sanitized, cap);
memcpy(player_name_out, sanitized, n);
player_name_out[n]= '\0';
} else {
const char* dflt= "You";
size_t n= strnlen(dflt, cap);
memcpy(player_name_out, dflt, n);
player_name_out[n]= '\0';
}
}
