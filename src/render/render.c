                 ...
                 int              score_count)
{
         ...
         i++)
    {
        ...
        if (!hs_name)
            hs_name = "";
        snprintf(display_scores[display_count].name,
                 sizeof(display_scores[display_count].name),
                 "%.*s",
                 (int)sizeof(display_scores[display_count].name) - 1,
                 hs_name);
         ...
         p++)
    {
        ...
        if (p == 0 && player_name != NULL)
            snprintf(
                tmp_name_buf, sizeof(tmp_name_buf), "%s (live)", player_name);
        else
    ...
    if (!scores_changed && score_count > 0 && scores)
    {
        for (int i = 0; i < score_count; i++)
        {
            if (last_scores[i].score != highscore_get_score(scores[i])
                || strcmp(last_scores[i].name, highscore_get_name(scores[i]))
                       != 0)
            {
                scores_changed = true;
                break;
    ...
    if (scores_changed && scores)
    {
        ...
        for (int i = 0; i < score_count && i < PERSIST_MAX_SCORES; i++)
        {
            ...
            if (!hs_name)
                hs_name = "";
            snprintf(last_scores[i].name,
                     sizeof(last_scores[i].name),
                     "%.*s",
                     (int)sizeof(last_scores[i].name) - 1,
                     hs_name);
    ...
    if (max_display > 0)
    {
             ...
             i++)
        {
            ...
            if (is_session_score_from_display(&display_scores[i])
...
void render_prompt_for_highscore_name(char* player_name_out,
                                      int   max_len,
                                      int   score)
{
    if (!g_display || !player_name_out || max_len <= 0)
    ...
    while (!name_confirmed)
    {
        ...
        int y = 1;
        draw_centered_string(
            y++, "CONGRATULATIONS!", DISPLAY_COLOR_BRIGHT_GREEN);
        draw_centered_string(
            y++, "You made the high score list!", DISPLAY_COLOR_BRIGHT_YELLOW);
        char score_str[32];
        snprintf(score_str, sizeof(score_str), "Score: %d", score);
        draw_centered_string(y++, score_str, DISPLAY_COLOR_CYAN);
        y++;
        draw_centered_string(y++,
                             "ENTER A SHORT NAME (max 8 chars)",
                             DISPLAY_COLOR_BRIGHT_YELLOW);
        y++;
        int box_width     = 20;
        int display_width = 0, display_height = 0;
        ...
        draw_centered_string(
            y++,
            "Press ENTER to confirm, leave blank to use default",
            DISPLAY_COLOR_WHITE);
