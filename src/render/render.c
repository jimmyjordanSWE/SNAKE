...
static bool is_session_score(const HighScore* s)
{
    ...
    for (int i = 0; i < g_session_score_count; i++)
        if (g_session_scores[i].score == s->score
            && strcmp(g_session_scores[i].name, s->name) == 0)
            return true;
    return false;
}
...
void render_note_session_score(const char* name, int score)
{
    if (!name || score <= 0)
        return;
    for (int i = 0; i < g_session_score_count; i++)
        if (g_session_scores[i].score == score
            && strcmp(g_session_scores[i].name, name) == 0)
            return;
    if (g_session_score_count >= SESSION_MAX_SCORES)
        return;
    snprintf(g_session_scores[g_session_score_count].name,
             sizeof(g_session_scores[g_session_score_count].name),
             "%s",
             name);
...
static void draw_box(int x, int y, int width, int height, uint16_t fg_color)
{
    ...
    if (x >= 0 && y >= 0 && x < display_width && y < display_height)
        display_put_char(g_display,
                         x,
                         y,
                         DISPLAY_CHAR_BOX_TL,
                         fg_color,
                         DISPLAY_COLOR_BLACK);
    if (x + width - 1 >= 0 && y >= 0 && x + width - 1 < display_width
        && y < display_height)
        display_put_char(g_display,
                         x + width - 1,
                         y,
                         ...
                         fg_color,
                         DISPLAY_COLOR_BLACK);
    if (x >= 0 && y + height - 1 >= 0 && x < display_width
        && y + height - 1 < display_height)
        display_put_char(g_display,
                         x,
                         y + height - 1,
                         DISPLAY_CHAR_BOX_BL,
    ...
    if (x + width - 1 >= 0 && y + height - 1 >= 0
        && x + width - 1 < display_width && y + height - 1 < display_height)
        display_put_char(g_display,
                         x + width - 1,
                         y + height - 1,
                         DISPLAY_CHAR_BOX_BR,
...
static void fill_background(void)
{
    ...
    for (int y = 0; y < display_height; y++)
        for (int x = 0; x < display_width; x++)
            display_put_char(
                g_display, x, y, ' ', DISPLAY_COLOR_BLACK, DISPLAY_COLOR_BLACK);
}
typedef struct
{
    bool up;
...
static uint16_t
glyph_for_segment_utf8(const SnakePoint* body, int length, int idx)
{
    ...
    if (links.left && links.right && !links.up && !links.down)
        return DISPLAY_CHAR_BOX_H;
    if (links.up && links.down && !links.left && !links.right)
        return DISPLAY_CHAR_BOX_V;
    if (links.up && links.right)
        return DISPLAY_CHAR_BOX_BL;
    if (links.up && links.left)
        return DISPLAY_CHAR_BOX_BR;
    if (links.down && links.right)
        return DISPLAY_CHAR_BOX_TL;
    if (links.down && links.left)
        return DISPLAY_CHAR_BOX_TR;
    return (uint16_t)'o';
}
static uint16_t
glyph_for_segment_ascii(const SnakePoint* body, int length, int idx)
{
    if (!body || length <= 0 || idx < 0 || idx >= length)
        return (uint16_t)'o';
    if (idx == 0)
    ...
    if (links.up && links.down && !links.left && !links.right)
        return (uint16_t)'|';
    if ((links.up && links.right) || (links.up && links.left)
        || (links.down && links.right) || (links.down && links.left))
        return (uint16_t)'+';
    return (uint16_t)'o';
}
                 ...
                 int              score_count)
{
    ...
    for (int i = 0; i < game->food_count; i++)
        display_put_char(g_display,
                         field_x + 1 + game->food[i].x,
                         field_y + 1 + game->food[i].y,
                         '*',
                         DISPLAY_COLOR_GREEN,
                         DISPLAY_COLOR_BLACK);
    for (int p = 0; p < game->num_players; p++)
    {
        ...
        uint16_t snake_color =
            (p == 0) ? DISPLAY_COLOR_BRIGHT_YELLOW : DISPLAY_COLOR_BRIGHT_RED;
        for (int i = 0; i < player->length; i++)
        {
            uint16_t ch = glyph_for_segment(player->body, player->length, i);
            display_put_char(g_display,
                             field_x + 1 + player->body[i].x,
                             field_y + 1 + player->body[i].y,
                             ch,
                             snake_color,
                             DISPLAY_COLOR_BLACK);
        }
        ...
        if (game->players[p].active)
        {
            ...
            if (p == 0 && player_name != NULL)
                ...
                snprintf(score_str,
                         ...
                         p + 1,
                         game->players[p].score);
            uint16_t color = (p == 0) ? DISPLAY_COLOR_BRIGHT_YELLOW
                                      : DISPLAY_COLOR_BRIGHT_RED;
            draw_string(
                field_x + field_width + 2, field_y + p * 2, score_str, color);
        }
    }
    if (!scores)
        score_count = 0;
    ...
    int       display_count = 0;
    for (int i = 0; i < score_count
                    && display_count < (int)(sizeof(display_scores)
                                             / sizeof(display_scores[0]));
         i++)
        display_scores[display_count++] = scores[i];
    for (int p = 0; p < game->num_players
                    && display_count < (int)(sizeof(display_scores)
                                             / sizeof(display_scores[0]));
         p++)
    {
    ...
    if (display_count > 1)
        qsort(display_scores,
              (size_t)display_count,
              sizeof(HighScore),
              compare_highscores_desc);
    ...
    if (!scores_changed && score_count > 0 && scores)
    {
        ...
        {
            if (last_scores[i].score != scores[i].score
                || strcmp(last_scores[i].name, scores[i].name) != 0)
            {
                scores_changed = true;
                break;
            }
    ...
    draw_string(field_x + field_width + 2,
                hiscore_y,
                "High Scores:",
                DISPLAY_COLOR_WHITE);
    if (max_display > 0)
    {
        for (int i = 0; i < max_display && hiscore_y + i + 1 < display_height;
             i++)
        {
            char hiscore_str[80];
            int  written =
                snprintf(hiscore_str, sizeof(hiscore_str), "%d. ", i + 1);
            if (written > 0 && written < (int)sizeof(hiscore_str))
                snprintf(hiscore_str + written,
                         sizeof(hiscore_str) - (size_t)written,
                         "%.15s: %d",
                         display_scores[i].name,
                         display_scores[i].score);
            uint16_t fg = gradient_color_for_rank(i, max_display);
            if (is_session_score(&display_scores[i])
                || strstr(display_scores[i].name, "(live)") != NULL)
                fg = DISPLAY_COLOR_BRIGHT_GREEN;
            draw_string(
                field_x + field_width + 2, hiscore_y + i + 1, hiscore_str, fg);
        }
    }
    else
    {
    ...
    if (display_height > field_y + field_height + 1)
        draw_string(
            1, field_y + field_height + 1, "Q: Quit", DISPLAY_COLOR_WHITE);
...
void render_draw_startup_screen(char* player_name_out, int max_len)
{
    ...
    while (!name_confirmed)
    {
        ...
        y++;
        draw_centered_string(
            y++, "A Game of Hunger & Growth", DISPLAY_COLOR_CYAN);
        draw_centered_string(
            y++, "Navigate. Consume. Dominate.", DISPLAY_COLOR_CYAN);
        y++;
        draw_centered_string(
            y++, "ENTER YOUR LEGEND'S NAME", DISPLAY_COLOR_BRIGHT_YELLOW);
        ...
        if (input_pos < 30)
            draw_string(
                box_x + 2 + input_pos, y, "_", DISPLAY_COLOR_BRIGHT_YELLOW);
        draw_string(box_x + box_width - 1, y, "|", DISPLAY_COLOR_BRIGHT_YELLOW);
        y++;
        draw_string(box_x, y, "+", DISPLAY_COLOR_BRIGHT_YELLOW);
        for (int i = 0; i < box_width - 2; i++)
        ...
        y += 2;
        draw_centered_string(
            y++, "MASTER THE CONTROLS", DISPLAY_COLOR_BRIGHT_YELLOW);
        draw_centered_string(y++, "", DISPLAY_COLOR_WHITE);
        draw_centered_string(
            y++, "  ^  Arrow Keys = Navigate", DISPLAY_COLOR_WHITE);
        draw_centered_string(y++, "< v >", DISPLAY_COLOR_WHITE);
        draw_centered_string(y++, "", DISPLAY_COLOR_WHITE);
        draw_centered_string(
            y++, "P = Pause   R = Restart   Q = Quit", DISPLAY_COLOR_WHITE);
        y++;
        draw_centered_string(
            y++, "Type name above, press ENTER to start", DISPLAY_COLOR_CYAN);
        display_present(g_display);
...
void render_draw_death_overlay(const GameState* game,
                               int              anim_frame,
                               bool             show_prompt)
{
    (void)anim_frame;
    ...
    if (show_prompt)
    {
        ...
        draw_string(
            box_x + 10, box_y + 5, "or Q to quit", DISPLAY_COLOR_BRIGHT_GREEN);
    }
    display_present(g_display);
}
