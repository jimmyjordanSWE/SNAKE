...
static void render_3d_draw_char(
    SDL3DContext* disp, int x, int y, char c, uint32_t col, int scale);
...
static void render_3d_draw_minimap(Render3DContext* r, float interp_t)
{
    ...
    int cell_px =
        render_3d_compute_minimap_cell_px(render_3d_sdl_get_width(r->display),
                                          render_3d_sdl_get_height(r->display),
                                          map_w,
                                          map_h);
    ...
    if (dbg_minimap_early && dbg_minimap_early[0] == '1')
    {
        render_3d_log("minimap: called gs=%p map=%dx%d display=%dx%d x0=%d "
                      ...
                      map_w,
                      map_h,
                      render_3d_sdl_get_width(r->display),
                      render_3d_sdl_get_height(r->display),
                      x0,
                      y0,
                      cell_px);
    ...
    if (dbg_minimap && dbg_minimap[0] == '1')
    {
        ...
        for (int xx = 0; xx < map_px_w; xx++)
        {
            ...
            render_3d_sdl_blend_pixel(
                r->display, x0 + xx, y0 + map_px_h - 1, dbgcol);
        }
        for (int yy = 0; yy < map_px_h; yy++)
        {
            render_3d_sdl_blend_pixel(r->display, x0, y0 + yy, dbgcol);
            render_3d_sdl_blend_pixel(
                r->display, x0 + map_px_w - 1, y0 + yy, dbgcol);
        }
        render_3d_log(
            "minimap: debug border drawn at %d,%d size %dx%d (display %dx%d)\n",
            x0,
            y0,
            map_px_w,
            map_px_h,
    ...
    uint32_t player_cols[3] = {render_3d_sdl_color(0, 128, 0, 255),
                               render_3d_sdl_color(0, 200, 255, 255),
    ...
    for (int p = 0; p < gs->num_players; p++)
    {
        ...
        for (int bi = 1; bi < pl->length; bi++)
        {
            ...
            {
                seg_x_f =
                    pl->prev_segment_x[bi]
                    + (((float)pl->body[bi].x + 0.5f) - pl->prev_segment_x[bi])
                          * interp_t;
                seg_y_f =
                    pl->prev_segment_y[bi]
                    + (((float)pl->body[bi].y + 0.5f) - pl->prev_segment_y[bi])
                          * interp_t;
            }
            else
            {
            ...
            int radius =
                bw
                / 2; /* keep radius similar to previous rectangle half-width */
            if (radius <= 0)
                radius = 1;
            render_3d_sdl_draw_filled_circle(
                r->display, tx, ty, radius, tail_col);
            /* Connections between segments removed to keep minimap segments as
             * circles only */
            (void)bi; /* suppress unused warning if any */
        }
        /* Then draw the interpolated head so it appears above any overlapping
         * body */
        float head_x = pl->prev_head_x
                       + (((float)pl->body[0].x + 0.5f) - pl->prev_head_x) * interp_t;
        float head_y = pl->prev_head_y
                       + (((float)pl->body[0].y + 0.5f) - pl->prev_head_y) * interp_t;
        int      hx = x0 + (int)(head_x * (float)cell_px + 0.5f);
        int      hy = y0 + (int)(head_y * (float)cell_px + 0.5f);
        int      hr = cell_px > 2 ? (cell_px / 2) : 1;
        uint32_t pcol =
            player_cols[p
                        % (int)(sizeof(player_cols) / sizeof(player_cols[0]))];
        ...
        render_3d_sdl_draw_filled_circle(r->display,
                                         hx + dir_off_x,
                                         hy + dir_off_y,
                                         hr > 1 ? hr / 2 : 1,
                                         pcol);
...
void render_3d_draw_minimap_into(struct SDL3DContext* ctx, const GameState* gs)
{
    ...
    tmp.display         = ctx;
    tmp.game_state      = gs;
    /* Use 0.0 for interpolation since this is a test helper with no camera state */
...
static void render_3d_draw_fps_counter(void)
{
    ...
    /* Draw in top-left corner with green text */
    ...
    /* Draw hundreds digit if needed */
    if (int_part >= 100)
    {
        render_3d_draw_char(g_render_3d.display,
                            x,
                            4,
                            (char)('0' + (int_part / 100)),
                            fps_color,
    ...
    /* Draw ones digit */
    render_3d_draw_char(
        g_render_3d.display, x, 4, (char)('0' + (int_part % 10)), fps_color, 1);
    x += 6;
    
    /* Draw decimal point and fraction */
 ...
 * Each glyph is 5 pixels wide and 7 pixels tall. Stored as 7 bytes (one per
 * row) with the least significant 5 bits used (LSB = leftmost pixel).
 */
...
static void render_3d_draw_text_centered(
    SDL3DContext* disp, int y, const char* text, uint32_t col, int scale)
{
    if (!disp || !text)
        return;
    int char_w    = 5 * scale;
                                  ...
                                  bool             show_prompt)
{
    ...
    render_3d_draw_text_centered(
        d, ly, "YOU DIED", render_3d_sdl_color(255, 255, 255, 255), 4);
    /* add an empty line (gap equal to previous line height) */
    ly += 4 * 7 + 4 * 7;
    if (show_prompt)
    {
        render_3d_draw_text_centered(d,
                                     ly,
                                     "PRESS ANY KEY TO RESTART",
                                     render_3d_sdl_color(160, 255, 160, 255),
                                     2);
        /* add an empty line equal to the previous small line height */
        ly += 2 * 7 + 2 * 7;
        render_3d_draw_text_centered(
            d, ly, "OR Q TO QUIT", render_3d_sdl_color(160, 255, 160, 255), 2);
    }
...
void render_3d_draw_congrats_overlay(int score, const char* name_entered)
{
    ...
    int ly = y + 6;
    render_3d_draw_text_centered(
        d, ly, "CONGRATULATIONS!", render_3d_sdl_color(255, 255, 0, 255), 3);
    /* empty line gap */
    ly += 3 * 7 + 3 * 7;
    char buf[64];
    snprintf(buf, sizeof(buf), "SCORE: %d", score);
    render_3d_draw_text_centered(
        d, ly, buf, render_3d_sdl_color(200, 200, 255, 255), 2);
    /* empty line gap */
    ly += 2 * 7 + 2 * 7;
    if (name_entered && name_entered[0])
    {
        ...
        render_3d_draw_text_centered(
            d, ly, nb, render_3d_sdl_color(160, 255, 160, 255), 2);
    }
    else
    {
        render_3d_draw_text_centered(d,
...
int render_3d_compute_minimap_cell_px(int d_w, int d_h, int m_w, int m_h)
{
    ...
    /* target minimap occupies a percentage of the smaller display dimension for
     * improved readability */
    int target = (smaller * MINIMAP_TARGET_OCCUPANCY_PERCENT) / 100;
    if (target < MINIMAP_MIN_TARGET_SIZE)
        target = MINIMAP_MIN_TARGET_SIZE; /* ensure reasonable visibility for large maps */
    int cell_px = target / max_dim;
    if (cell_px < MINIMAP_MIN_CELL_PIXELS)
        cell_px = MINIMAP_MIN_CELL_PIXELS; /* prefer at least this many pixels per cell for readability */
    return cell_px;
}
...
bool render_3d_init(const GameState* game_state, const Render3DConfig* config)
{
    ...
    else
    {
        ...
        g_render_3d.config.wall_height_scale =
            (float)PERSIST_CONFIG_DEFAULT_WALL_SCALE;
        g_render_3d.config.tail_height_scale =
            (float)PERSIST_CONFIG_DEFAULT_TAIL_SCALE;
        g_render_3d.config.wall_texture_path[0]  = '\0';
        g_render_3d.config.floor_texture_path[0] = '\0';
    }

    g_render_3d.display = render_3d_sdl_create(
        g_render_3d.config.screen_width, g_render_3d.config.screen_height);
    if (!g_render_3d.display)
        return false;
    g_render_3d.camera = camera_create(
        g_render_3d.config.fov_degrees, g_render_3d.config.screen_width, 0.5f);
    if (!g_render_3d.camera)
        return false;
    g_render_3d.raycaster =
        raycaster_create(game_state->width, game_state->height, NULL);
    if (!g_render_3d.raycaster)
        return false;
    g_render_3d.projector = projection_create(
        g_render_3d.config.screen_width,
        g_render_3d.config.screen_height,
    ...
    {
        if (!texture_load_from_file(g_render_3d.wall_texture,
    ...
    {
        if (!texture_load_from_file(g_render_3d.floor_texture,
                                    g_render_3d.config.floor_texture_path))
        {
            fprintf(
                stderr,
                "render_3d_init: failed to load %s (using flat floor color)\n",
                g_render_3d.config.floor_texture_path);
        }
    }
    else
    ...
    g_render_3d.column_depths = calloc(
        (size_t)render_3d_sdl_get_width(g_render_3d.display), sizeof(float));
    if (!g_render_3d.column_depths)
        return false;
    g_render_3d.sprite_renderer =
        sprite_create(100, g_render_3d.camera, g_render_3d.projector);
                    ...
                    float            delta_seconds)
{
    ...
    /* Cache interpolation fraction once for the entire frame to ensure all
     * entities move smoothly with consistent timing. Using the same t value
     * for camera, players, and objects prevents jitter from multiple
    ...
    const int   map_w =
        g_render_3d.game_state ? g_render_3d.game_state->width : 0;
    const int map_h =
        g_render_3d.game_state ? g_render_3d.game_state->height : 0;

    /* Precompute per-scanline floor distances so we don't redo divides for
     * every column. This is the main performance win for textured floors.
     * Use a more robust realloc pattern with size validation. */
    static float* s_floor_row_dist     = NULL;
    static int    s_floor_row_dist_cap = 0;
    static bool   s_floor_row_dist_valid = false;
    
    /* Only reallocate if size changes, not on every frame */
    if (screen_h != s_floor_row_dist_cap || !s_floor_row_dist_valid)
    {
        float* new_buf =
            (float*)realloc(s_floor_row_dist, (size_t)screen_h * sizeof(float));
        if (new_buf)
        {
        ...
        {
            /* Allocation failed - mark as invalid and skip floor distance cache
             * this frame (fallback to flat color rendering) */
            s_floor_row_dist_valid = false;
        }
    }
    
    /* Only use floor distance cache if allocation succeeded */
    ...
    if (dbg && dbg[0] == '1')
    {
        ...
        if (wp)
        {
            for (int yy = 0; yy < 16; yy++)
            {
                ...
                {
                    uint32_t c = texture_sample(g_render_3d.wall_texture,
                                                (float)xx / 16.0f,
                                                (float)yy / 16.0f,
                                                true);
                    if (8 + xx >= 0
                        && 8 + xx < render_3d_sdl_get_width(g_render_3d.display)
                        && 8 + yy >= 0
                        && 8 + yy
                               < render_3d_sdl_get_height(g_render_3d.display))
                        render_3d_sdl_get_pixels(
                            g_render_3d.display)[(8 + yy)
        ...
        if (fp)
        {
            for (int yy = 0; yy < 16; yy++)
            {
                for (int xx = 0; xx < 16; xx++)
                {
                    uint32_t c = texture_sample(g_render_3d.floor_texture,
                                                (float)xx / 16.0f,
                                                (float)yy / 16.0f,
                                                true);
                    if (8 + xx >= 0
    ...
    camera_get_interpolated_position(
        g_render_3d.camera, &interp_cam_x, &interp_cam_y);
    ...
    for (int x = 0; x < screen_w; x++)
    {
        ...
        if (raycast_cast_ray(
                g_render_3d.raycaster, origin_x, origin_y, ray_angle, &hit))
        {
            WallProjection proj;
            ...
            projection_project_wall_perp(g_render_3d.projector,
                                         hit.distance,
                                         ray_angle,
                                         interp_cam_angle,
                                         &proj);
            render_3d_sdl_draw_column(
                g_render_3d.display, x, 0, proj.draw_start - 1, ceiling_color);
            Texel texel;
            ...
            /* raycast_get_texture_coord returns value in [0,1] range.
             * Do NOT multiply by TEXTURE_SCALE - texture_sample expects [0,1] */
            ...
            for (int yy = proj.draw_start; yy <= proj.draw_end; yy++)
            {
                ...
                float    tex_v = v;  /* Normalized [0,1] for proper texture sampling */
                uint32_t col   = 0;
                if (texture_has_image(g_render_3d.wall_texture))
                {
                    col = texture_sample(
                        g_render_3d.wall_texture, tex_coord, tex_v, true);
                }
                else
                {
                    texture_get_texel(g_render_3d.texture,
                                      pd,
                                      hit.is_vertical,
                                      tex_coord,
                                      &texel);
            ...
            if (fh0 <= fh1)
            {
                if (has_floor_tex)
                {
                    ...
                    {
                        float rowDist =
                            s_floor_row_dist ? s_floor_row_dist[yy] : 0.0f;
                        if (rowDist <= 0.0f)
                            continue;
                        float world_x = interp_cam_x + cos_a * rowDist;
                        float world_y = interp_cam_y + sin_a * rowDist;
                        /* Skip sampling when world position is outside the map
                         * bounds to avoid drawing floor beyond the playable
                         * area. */
                        if (g_render_3d.game_state)
                        {
                            if (world_x < 0.0f || world_x >= (float)map_w
                                || world_y < 0.0f || world_y >= (float)map_h)
                            {
                                if (pix)
                        ...
                        /* Map world coords to texture space. Multiply by
                         * TEXTURE_SCALE so each world unit corresponds to a
                         * tile of texture pixels. */
                        float u = world_x * (float)TEXTURE_SCALE;
                        float v = world_y * (float)TEXTURE_SCALE;
                        /* Nearest sampling is much cheaper and sufficient for
                         * the floor. */
                        uint32_t col = texture_sample(
                            g_render_3d.floor_texture, u, v, false);
                        if (pix)
                            pix[yy * screen_w + x] = col;
                    }
                }
                ...
                {
                    render_3d_sdl_draw_column(
                        g_render_3d.display, x, fh0, fh1, floor_color);
                }
            }
            else
            {
                ...
                render_3d_sdl_draw_column(
                    g_render_3d.display, x, 0, horizon - 1, ceiling_color);
                int fh0b = horizon;
                int fh1b = screen_h - 1;
                if (fh0b <= fh1b)
                {
                    ...
                    {
                        render_3d_sdl_draw_column(
                            g_render_3d.display, x, fh0b, fh1b, floor_color);
                    }
                }
            }
        }
    ...
    /* Phase 2: Draw all sprites (once per frame, not per column) */
    ...
    for (int i = 0; i < game_state->food_count; i++)
    {
        ...
        sprite_add_color(g_render_3d.sprite_renderer,
                         (float)game_state->food[i].x + 0.5f,
                         (float)game_state->food[i].y + 0.5f,
                         0.25f,
                         0.0f,
                         true,
                         -1,
    ...
    /* Add other players' head sprites */
    for (int p = 0; p < game_state->num_players; p++)
    {
        ...
        float head_x =
            player->prev_head_x
            + (((float)player->body[0].x + 0.5f) - player->prev_head_x) * frame_interp_t;
        float head_y =
            player->prev_head_y
            + (((float)player->body[0].y + 0.5f) - player->prev_head_y) * frame_interp_t;
        uint32_t player_cols[3] = {render_3d_sdl_color(0, 128, 0, 255),
                                   render_3d_sdl_color(0, 200, 255, 255),
                                   render_3d_sdl_color(255, 255, 0, 255)};
        uint32_t pcol           = player_cols[p
                                    % (int)(sizeof(player_cols)
                                            / sizeof(player_cols[0]))];
        /* Draw head as a colored circle sprite (texture not sampled yet).
         * Use texture_id = -1 so it follows the circle-rendering path
         * instead of drawing a solid textured column (which previously
         * showed as a green square when textures were not used). */
        sprite_add_color(g_render_3d.sprite_renderer,
                         head_x,
                         head_y,
                         1.0f,
        ...
        for (int bi = 1; bi < player->length; bi++)
        {
            ...
            if (player->prev_segment_x && player->prev_segment_y)
            {
                seg_x = player->prev_segment_x[bi]
                        + (((float)player->body[bi].x + 0.5f)
                           - player->prev_segment_x[bi])
                              * frame_interp_t;
                seg_y = player->prev_segment_y[bi]
                        + (((float)player->body[bi].y + 0.5f)
                           - player->prev_segment_y[bi])
                              * frame_interp_t;
            }
            float    tail_h = g_render_3d.config.tail_height_scale;
            uint32_t body_col = render_3d_sdl_color(
                0, 128, 0, 255);  /* Green for body segments */
...
void render_3d_on_tick(const GameState* game_state)
{
    ...
    if (g_render_3d.config.active_player < game_state->num_players)
    {
        const PlayerState* player =
            &game_state->players[g_render_3d.config.active_player];
        if (player->length > 0)
            camera_set_from_player(g_render_3d.camera,
                                   player->body[0].x,
                                   player->body[0].y,
                                   player->current_dir);
    }
...
void render_3d_set_tick_rate_ms(int ms)
{
    ...
    if (camera_get_interp_time(g_render_3d.camera)
        > camera_get_update_interval(g_render_3d.camera))
        camera_set_interpolation_time(
            g_render_3d.camera, camera_get_update_interval(g_render_3d.camera));
}
