...
tmp.display= ctx;
tmp.game_state= gs;
/* Use 0.0 for interpolation since this is a test helper with no camera
     * state */
...
for(int xx= 0; xx < scale; xx++) render_3d_sdl_blend_pixel(disp, px + xx, py + yy, col);
}
}
}
}
...
target= MINIMAP_MIN_TARGET_SIZE; /* ensure reasonable visibility for
                                             large maps */
int cell_px= target / max_dim;
if(cell_px < MINIMAP_MIN_CELL_PIXELS)
cell_px= MINIMAP_MIN_CELL_PIXELS; /* prefer at least this many pixels
                                              per cell for readability */
return cell_px;
}
...
void render_3d_draw(const GameState* game_state, const char* player_name, const void* scores, int score_count, float delta_seconds) {
    ...
    if(clamped_delta > MAX_DELTA_SECONDS) clamped_delta= MAX_DELTA_SECONDS;
    ...
    if(s_floor_row_dist && s_floor_row_dist_valid) {
        ...
        for(int yy= 0; yy < screen_h; yy++) {
            ...
            /* The floor distance must be the inverse of the wall projection
             * formula used in projection.c: height = (screen_h / (dist + 0.5)) * scale.
             * This ensures the floor texture aligns perfectly with the wall bases. */
    ...
    for(int x= 0; x < screen_w; x++) {
        ...
        if(raycast_cast_ray(g_render_3d.raycaster, origin_x, origin_y, ray_angle, &hit)) {
            ...
            for(int yy= proj.draw_start; yy <= proj.draw_end; yy++) {
                ...
                float tex_v= v * g_render_3d.config.wall_texture_scale; /* Use wall_texture_scale for vertical tiling too */
                uint32_t col= 0;
                ...
                if(has_floor_tex) {
                    for(int yy= fh0; yy <= fh1; yy++) {
                        ...
                        if(world_x < 0.0f || world_x >= (float)map_w || world_y < 0.0f || world_y >= (float)map_h) {
                            if(pix) pix[yy * screen_w + x]= floor_color;
                            continue;
                        }
                        /* Map world coords to texture space. */
    ...
    for(int i= 0; i < game_state->food_count; i++) {
        ...
        sprite_add_color(g_render_3d.sprite_renderer, (float)game_state->food[i].x + 0.5f, (float)game_state->food[i].y + 0.5f, 0.25f, 0.0f, true, -1, 0, apple_col);
    }
    ...
    for(int p= 0; p < game_state->num_players; p++) {
        ...
        for(int bi= 1; bi < player->length; bi++) {
            ...
            if(player->prev_segment_x && player->prev_segment_y) {
                seg_x= player->prev_segment_x[bi] + (((float)player->body[bi].x + 0.5f) - player->prev_segment_x[bi]) * frame_interp_t;
                seg_y= player->prev_segment_y[bi] + (((float)player->body[bi].y + 0.5f) - player->prev_segment_y[bi]) * frame_interp_t;
            }
            float tail_h= g_render_3d.config.tail_height_scale;
            uint32_t body_col= render_3d_sdl_color(0, 128, 0, 255); /* Green for body segments */
    ...
    if(!render_3d_sdl_present(g_render_3d.display)) { /* Rendering failed - likely user closed window */ }
