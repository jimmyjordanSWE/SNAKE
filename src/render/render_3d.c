...
bool render_3d_init(const GameState* game_state, const Render3DConfig* config)
{
    ...
    fprintf(stderr,
            "render_3d_init: requested size %dx%d\n",
            g_render_3d.config.screen_width,
            g_render_3d.config.screen_height);
    if (!render_3d_sdl_init(g_render_3d.config.screen_width,
                            g_render_3d.config.screen_height,
                            &g_render_3d.display))
        return false;
    camera_init(&g_render_3d.camera,
                g_render_3d.config.fov_degrees,
                g_render_3d.config.screen_width,
                0.5f);
    raycast_init(
        &g_render_3d.raycaster, game_state->width, game_state->height, NULL);
    /* projection with configurable wall height scale */
    projection_init(&g_render_3d.projector,
                    g_render_3d.config.screen_width,
                    g_render_3d.config.screen_height,
    ...
    g_render_3d.column_depths =
        calloc((size_t)g_render_3d.display.width, sizeof(float));
    sprite_init(&g_render_3d.sprite_renderer,
                100,
                &g_render_3d.camera,
                &g_render_3d.projector);
                    ...
                    float            delta_seconds)
{
            ...
            for (int yy = 0; yy < 16; yy++)
            {
                for (int xx = 0; xx < 16; xx++)
                {
                    uint32_t c = texture_sample(&g_render_3d.wall_texture,
                                                ...
                                                (float)yy / 16.0f,
                                                true);
                    if (8 + xx >= 0 && 8 + xx < g_render_3d.display.width
                        && 8 + yy >= 0 && 8 + yy < g_render_3d.display.height)
                        g_render_3d.display
                            .pixels[(8 + yy) * g_render_3d.display.width
                                    + (8 + xx)] = c;
                }
            }
        }
            ...
            for (int yy = 0; yy < 16; yy++)
            {
                for (int xx = 0; xx < 16; xx++)
                {
                    uint32_t c = texture_sample(&g_render_3d.floor_texture,
                                                ...
                                                true);
                    if (8 + xx >= 0 && 8 + xx < g_render_3d.display.width
                        && 28 + yy >= 0 && 28 + yy < g_render_3d.display.height)
                        g_render_3d.display
                            .pixels[(28 + yy) * g_render_3d.display.width
                                    + (8 + xx)] = c;
                }
            }
        }
    ...
    for (int x = 0; x < g_render_3d.display.width; x++)
    {
        ...
        */
                ...
                &g_render_3d.raycaster, origin_x, origin_y, ray_angle, &hit))
        {
            ...
            {
                float v =
                    (float)(yy - proj.draw_start) / (float)wall_h; /* 0..1 */
                float    tex_v = v * (float)TEXTURE_SCALE;
                uint32_t col   = 0;
                if (g_render_3d.wall_texture.pixels
                    && g_render_3d.wall_texture.img_w > 0
                    && g_render_3d.wall_texture.img_h > 0)
                {
                    col = texture_sample(
                ...
                if (pix && x >= 0 && x < w && yy >= 0 && yy < h)
                    pix[yy * w + x] = col;
            }
            /* floor: Draw solid brown color below walls */
            render_3d_sdl_draw_column(&g_render_3d.display,
                                      x,
                                      proj.draw_end,
                                      g_render_3d.display.height - 1,
                                      floor_color);
        }
        else
        {
    ...
    for (int i = 0; i < game_state->food_count; i++)
    {
        ...
        sprite_add(&g_render_3d.sprite_renderer,
    ...
    for (int p = 0; p < game_state->num_players; p++)
    {
        ...
        {
            t = g_render_3d.camera.interp_time
                / g_render_3d.camera.update_interval;
            if (t < 0.0f)
                t = 0.0f;
            if (t > 1.0f)
                t = 1.0f;
        ...
        float head_x =
            player->prev_head_x
            + (((float)player->body[0].x + 0.5f) - player->prev_head_x) * t;
        float head_y =
            player->prev_head_y
            + (((float)player->body[0].y + 0.5f) - player->prev_head_y) * t;
        sprite_add(&g_render_3d.sprite_renderer,
                   head_x,
                   head_y,
                   1.0f,
...
void render_3d_set_tick_rate_ms(int ms)
{
    ...
    if (g_render_3d.camera.interp_time > g_render_3d.camera.update_interval)
        g_render_3d.camera.interp_time = g_render_3d.camera.update_interval;
}
