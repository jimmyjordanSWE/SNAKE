...
#define MAX_DELTA_SECONDS 0.5f
    ...
    const int map_w = g_render_3d.game_state ? g_render_3d.game_state->width : 0;
    const int map_h = g_render_3d.game_state ? g_render_3d.game_state->height : 0;
                ...
                if (has_floor_tex) {
                    for (int yy = fh0; yy <= fh1; yy++) {
                        ...
                        if (g_render_3d.game_state) {
                            if (world_x < 0.0f || world_x >= (float)map_w || world_y < 0.0f ||
                                world_y >= (float)map_h) {
                                if (pix)
