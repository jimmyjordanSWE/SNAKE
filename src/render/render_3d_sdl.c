...
bool render_3d_sdl_init(int width, int height, SDL3DContext* ctx_out)
{
    ...
    SDL_PumpEvents();
    ctx_out->renderer = SDL_CreateRenderer(ctx_out->window,
                                           -1,
    ...
    ctx_out->texture = SDL_CreateTexture(ctx_out->renderer,
...
void render_3d_sdl_blend_pixel(SDL3DContext* ctx,
                               int           x,
                               int           y,
                               uint32_t      src_col)
{
    ...
    ctx->pixels[y * ctx->width + x] = (0xFFu << 24) | ((uint32_t)out_r << 16)
                                      | ((uint32_t)out_g << 8)
                                      | (uint32_t)out_b;
}
...
void render_3d_sdl_draw_filled_rect(
    SDL3DContext* ctx, int x, int y, int w_rect, int h_rect, uint32_t col)
{
...
bool render_3d_sdl_present(SDL3DContext* ctx)
{
    ...
    SDL_UpdateTexture(ctx->texture,
                      NULL,
                      ctx->pixels,
                      ctx->width * (int)sizeof(uint32_t));
