 ...
 *
 * Perform a runtime lookup of the LSAN symbols with dlsym so we don't
 * create direct symbol references that would break linking in release
 * builds without LeakSanitizer. Link with -ldl to enable dlsym.
 */
#include <dlfcn.h>
...
if(ctx_out->initialized) return true; /* Already initialized */
...
ctx_out->texture= SDL_CreateTexture(ctx_out->renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, width, height);
if(!ctx_out->texture) {
...
/* Fast path for opaque colors - no blending needed */
if(alpha == 255) {
...
/* Blending path for transparent colors */
for(int yy= y0; yy <= y1; ++yy) {
