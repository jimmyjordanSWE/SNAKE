...
bool            render_3d_sdl_init(int width, int height, SDL3DContext* ctx_out)
{
    ...
    SDL_PumpEvents();
    g_sdl_state.renderer = SDL_CreateRenderer(g_sdl_state.window,
                                              -1,
    ...
    g_sdl_state.texture = SDL_CreateTexture(g_sdl_state.renderer,
...
{
    if (!ctx || !g_sdl_state.renderer || !g_sdl_state.texture)
        return false;
    SDL_PumpEvents();
    SDL_Event event;
    ...
    SDL_UpdateTexture(g_sdl_state.texture,
                      NULL,
                      ctx->pixels,
