...
int j= i - 1;
while(j >= 0 && (sr->sprites[j].perp_distance < key.perp_distance || (fabsf(sr->sprites[j].perp_distance - key.perp_distance) < 1e-4f && sr->sprites[j].texture_id != -1 && key.texture_id == -1))) {
