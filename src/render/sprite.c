...
void sprite_draw(SpriteRenderer3D* sr, SDL3DContext* ctx, const float* column_depths) {
    ...
    for (int i = 0; i < sr->count; ++i) {
        ...
        if (s->texture_id == -1) {
            ...
            if (!any_visible)
                continue; /* Skip fully occluded sprite */
                        ...
                        if (dx * dx + dy2 <= r2) {
                            if (s->perp_distance < column_depths[xx]) {
                                if (s->shaded) {
                                    ...
                                    if (xi < 0)
                                        xi = 0;
                                    if (xi >= LIGHT_TABLE_SIZE)
                                        xi = LIGHT_TABLE_SIZE - 1;
                                    if (yi < 0)
                                        yi = 0;
                                    if (yi >= LIGHT_TABLE_SIZE)
                                        yi = LIGHT_TABLE_SIZE - 1;
                                    float intensity = lighting_table[yi][xi];
                                    if (intensity <= 0.0f)
                                        continue; /* Outside sphere in table */
