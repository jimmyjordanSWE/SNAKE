...
void sprite_project_all(SpriteRenderer3D* sr)
{
    ...
    for (int i = 0; i < sr->count; ++i)
    {
        ...
        projection_project_wall_perp(
            sr->proj, perp, angle_to_sprite, cam_angle, &wp);
        ...
        /* If this sprite is a rectangle, make it wider than its height for a
         * better snake-like appearance in the 3D view. */
        if (s->is_rect)
        {
        ...
        if (nx > 1.0f)
            nx = 1.0f;
        int center_x = (int)((nx + 1.0f) * 0.5f
                                 * (float)projection_get_screen_width(sr->proj)
                             + 0.5f);
    ...
    for (int i = 0; i < sr->count; ++i)
    {
        ...
        for (int j = 0; j < sr->count; ++j)
        {
            ...
            if (b->texture_id != -1)
                continue; /* only look for tails */
            if (fabsf(a->world_x - b->world_x) < 1e-3f
                && fabsf(a->world_y - b->world_y) < 1e-3f)
            {
...
void sprite_sort_by_depth(SpriteRenderer3D* sr)
{
    ...
    for (int i = 1; i < sr->count; ++i)
    {
        ...
        while (
            j >= 0
            && (
                /* primary sort by descending perpendicular distance (far ->
                   near) */
                sr->sprites[j].perp_distance < key.perp_distance ||
                /* tie-break: when distances are equal, ensure tails (texture_id
                 * == -1) are placed before heads (texture_id != -1) so heads
                 * draw on top. Shift existing element when it is a head and the
                 * key is a tail. */
    ...
    while (changed)
    {
        ...
        for (int k = 0; k + 1 < sr->count; ++k)
        {
            ...
            if (a->texture_id != -1 && b->texture_id == -1
                && fabsf(a->world_x - b->world_x) < 0.6f
                 ...
                 const float*      column_depths)
{
    ...
    for (int i = 0; i < sr->count; ++i)
    {
        ...
        uint32_t col =
            s->color ? s->color : render_3d_sdl_color(0, 128, 0, 255);
        if (s->texture_id == -1)
        {
...
SpriteRenderer3D*
sprite_create(int max_sprites, const Camera3D* camera, const Projection3D* proj)
{
...
int sprite_get_texture_id(const SpriteRenderer3D* sr,
                          int                     idx,
                          int*                    texture_id_out)
{
    if (!sr || idx < 0 || idx >= sr->count)
