...
void texture_get_texel(const Texture3D* tex, float distance, bool is_vertical, float tex_coord, Texel* texel_out) {
    ...
    /* add a small pattern-dependent offset to the shade to make textures visible */
...
bool texture_load_from_file(Texture3D* tex, const char* filename) {
    ...
    /* Try a few candidate paths to be tolerant to different working directories. */
    ...
    /* 2b) try relative to the executable directory (helpful when the working dir is different) */
    if(!data) {
        char exe_path[1024]; exe_path[0] = '\0';
    ...
    if(!data) {
        ...
        fprintf(stderr, "texture_load_from_file: failed to load '%s' (stb_image returned NULL). Existence check:\n", filename);
        char check[1024];
        FILE* f = fopen(filename, "rb");
        fprintf(stderr, "  '%s' -> %s\n", filename, f ? "exists" : "missing"); if(f) fclose(f);
        if((size_t)snprintf(check, sizeof(check), "./%s", filename) < sizeof(check)) {
            f = fopen(check, "rb"); fprintf(stderr, "  '%s' -> %s\n", check, f ? "exists" : "missing"); if(f) fclose(f);
        }
        /* check parent directories */
        for(int depth = 1; depth <= 6; depth++) {
            size_t used = 0;
            for(int i = 0; i < depth && used + 3 < sizeof(check); i++) { check[used++] = '.'; check[used++] = '.'; check[used++] = '/'; }
            check[used] = '\0';
        ...
        /* check basename */
        const char* base = strrchr(filename, '/'); if(base) base++; else base = filename;
        f = fopen(base, "rb"); fprintf(stderr, "  '%s' -> %s\n", base, f ? "exists" : "missing"); if(f) fclose(f);
        return false;
    }
    if(try_path) {
        fprintf(stderr, "texture_load_from_file: loaded '%s' (resolved '%s')\n", filename, try_path);
    }
    ...
    if(!tex->pixels) { stbi_image_free(data); return false; }
    /* data is in R G B A order per pixel */
    /* SDL_PIXELFORMAT_ARGB8888 expects bytes [B, G, R, A] in little-endian memory,
       but a hex value 0xAARRGGBB becomes [BB, GG, RR, AA]. This swaps R and B.
       To fix this, we store as 0xAABBGGRR which becomes [RR, GG, BB, AA] in memory. */
    for(int y = 0; y < h; y++) {
        for(int x = 0; x < w; x++) {
            int i = (y * w + x) * 4;
...
static uint32_t sample_bilinear(const Texture3D* tex, float u, float v) {
    ...
    int x0 = (int)floorf(x); int x1 = x0 + 1; if(x1 >= tex->img_w) x1 = tex->img_w - 1;
    int y0 = (int)floorf(y); int y1 = y0 + 1; if(y1 >= tex->img_h) y1 = tex->img_h - 1;
    float sx = x - (float)x0; float sy = y - (float)y0;
