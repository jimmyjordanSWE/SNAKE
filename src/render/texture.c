...
void texture_init(Texture3D* tex)
{
    ...
    const uint32_t default_shades[TEXTURE_MAX_SHADES] = {
        0xFFEEEEEE, 0xFFCCCCCC, 0xFF999999, 0xFF666666, 0xFF333333, 0xFF000000};
    memcpy(tex->shade_colors, default_shades, sizeof(tex->shade_colors));
    const uint32_t side_v[TEXTURE_MAX_SHADES] = {
        0xFFDDDDDD, 0xFFBFBFBF, 0xFF8F8F8F, 0xFF606060, 0xFF303030, 0xFF000000};
    const uint32_t side_h[TEXTURE_MAX_SHADES] = {
        0xFFCCCCCC, 0xFFAAAAAA, 0xFF777777, 0xFF505050, 0xFF282828, 0xFF000000};
...
bool texture_load_from_file(Texture3D* tex, const char* filename)
{
    ...
    if (!data)
    {
        ...
        if (len > 0)
        {
            ...
            if (last)
                *last = '\0';
            if ((size_t)snprintf(buf, sizeof(buf), "%s/%s", exe_path, filename)
                < sizeof(buf))
            {
                data = stbi_load(buf, &w, &h, &channels, 4);
                if (data)
                { /* loaded from exe_path */
                }
            }
        }
    }
    for (int depth = 1; depth <= max_parent_depth && !data; depth++)
    {
        ...
        data = stbi_load(buf, &w, &h, &channels, 4);
        if (data)
        { /* loaded from parent candidate path */
        }
    }
    if (!data)
    {
    ...
    if (!data)
    {
        ...
        if (f)
            fclose(f);
        if ((size_t)snprintf(check, sizeof(check), "./%s", filename)
            < sizeof(check))
        {
            f = fopen(check, "rb");
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            tex->pixels[y * w + x] = ((uint32_t)a << 24) | ((uint32_t)b << 16)
