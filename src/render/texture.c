...
for(int y= 0; y < h; y++) {
for(int x= 0; x < w; x++) { t->pixels[y * w + x]= 0xFF000000 | ((uint32_t)(y & 0xFF) << 8) | (uint32_t)(x & 0xFF); }
}
...
if(filename[0] == '/') return false; /* absolute paths not allowed */
if(strstr(filename, "..") != NULL) return false; /* reject parent traversal */
/* reasonable length cap */
if(strlen(filename) == 0 || strlen(filename) >= 512) return false;
return true;
}
bool texture_load_from_file(Texture3D* tex, const char* filename) {
if(!tex || !filename) return false;
if(!is_safe_texture_filename(filename)) return false;
...
if(n < 0 || (size_t)n >= avail) break; /* truncated or error */
data= stbi_load(buf, &w, &h, &channels, 4);
if(data) {}
}
if(!data) {
...
static inline bool uv_in_unit_range(float u, float v) { return u >= 0.0f && u < 1.0f && v >= 0.0f && v < 1.0f; }
...
if(uv_in_unit_range(u, v)) return sample_nearest_norm(tex, u, v);
...
if(uv_in_unit_range(u, v) && tex->img_w > 1 && tex->img_h > 1) return sample_bilinear_fast(tex, u, v);
