...
float render_3d_compute_wall_u(float interp_cam_x, float interp_cam_y, float ray_angle, int yy, int horizon, int screen_height, float pd, bool is_vertical, float wall_world_height, float* out_rowDist) {
    ...
    if(fabsf(p) > 1e-6f) {
         ...
         * and derive z if needed. To support simple checks, if wall_world_height
         * is positive we assume caller wants the midpoint z for this yy:
         */
    ...
    if(out_rowDist) *out_rowDist = rowDist;
    float world_x = interp_cam_x + cosf(ray_angle) * rowDist;
    float world_y = interp_cam_y + sinf(ray_angle) * rowDist;
    float u_frac;
    if(is_vertical) u_frac = world_y - floorf(world_y);
    else u_frac = world_x - floorf(world_x);
