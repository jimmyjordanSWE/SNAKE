...
n= snprintf(buf + len, cap - len, "%s{\"id\":%d,\"name\":\"%s\",\"x\":%d,\"y\":%d,\"len\":%d,\"score\":%d}", (len > 0 && buf[len - 1] != '[') ? "," : "", i, escaped, hx, hy, p->length, p->score);
free(escaped);
if(n < 0) {
free(buf);
return NULL;
...
if(strstr(mpbuf, "\"type\":\"state\"")) parse_remote_game_state(mpbuf);
}
...
for(int i= 0; i < num_players && i < SNAKE_MAX_PLAYERS; ++i) (void)game_enqueue_input(game, i, &inputs[i]);
if(s->autoplay && (tick % 3) == 0) {
