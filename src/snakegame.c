...
SnakeGame* snake_game_new(const GameConfig* config_in, int* err_out)
{
    ...
    render_set_glyphs((game_config_get_render_glyphs(config_in) == 1)
                          ? RENDER_GLYPHS_ASCII
                          : RENDER_GLYPHS_UTF8);
    input_set_key_bindings(game_config_get_key_up(config_in),
                           game_config_get_key_down(config_in),
                           game_config_get_key_left(config_in),
                           game_config_get_key_right(config_in),
                           game_config_get_key_quit(config_in),
                           game_config_get_key_restart(config_in),
    ...
    while (1)
    {
        ...
        if (tty_size_sufficient_for_board(
                term_w, term_h, board_width, board_height))
            break;
        console_box_too_small_for_game(
            term_w, term_h, board_width + 4, board_height + 4);
        platform_sleep_ms(500);
        (void)platform_was_resized();
    }
    console_info(
        "Terminal size OK (%dx%d). Starting game...\n", term_w, term_h);
    ...
    config_3d.show_sprite_debug =
        (game_config_get_show_sprite_debug(config_in) != 0);
    ...
    if (game_config_get_wall_texture(config_in)
        && game_config_get_wall_texture(config_in)[0])
        snprintf(config_3d.wall_texture_path,
                 (int)sizeof(config_3d.wall_texture_path),
                 "%s",
                 game_config_get_wall_texture(config_in));
    if (game_config_get_floor_texture(config_in)
        && game_config_get_floor_texture(config_in)[0])
    ...
    render_draw(
        game_get_state(game), game_config_get_player_name(s->cfg), NULL, 0);
    s->game   = game;
    s->has_3d = has_3d;
    if (err_out)
        *err_out = 0;
...
int snake_game_run(SnakeGame* s)
{
    ...
    render_draw(game_get_state(game),
                game_config_get_player_name(cfg),
                highscores,
                highscore_count);
    while (((const struct GameState*)game_get_state(game))->status
           != GAME_STATUS_GAME_OVER)
    {
        if (platform_was_resized())
        {
            ...
            if (!tty_size_sufficient_for_board(
                    new_w, new_h, board_width, board_height))
            {
                render_draw(game_get_state(game),
                            game_config_get_player_name(cfg),
                            highscores,
                ...
                console_box_paused_terminal_small(
                    new_w, new_h, board_width + 4, board_height + 4);
                while (1)
                {
                    InputState in = {0};
                    input_poll(&in);
                    ...
                    if (tty_size_sufficient_for_board(
                            check_w, check_h, board_width, board_height))
                    {
        ...
        /* Track whether a high score prompt was shown for player 0 during this
         * tick. If so, restart immediately instead of waiting for key input so
         * the game doesn't quit. */
        ...
        for (int ei = 0; ei < events.died_count; ei++)
        {
            ...
            if (death_score > 0)
            {
                /* Only prompt for a name when the score would enter the high
                 * score list. */
                HighScore** cur = NULL;
                ...
                highscore_count =
                    persist_read_scores(".snake_scores", &highscores);
            }
        }
        ...
        if (player0_died)
        {
            ...
            /* If the player just entered a highscore, restart immediately to
             * avoid an accidental quit from pressing the quit key. */
        ...
        uint64_t frame_deadline =
            frame_start + (uint64_t)game_config_get_tick_rate_ms(cfg);
        ...
        while (platform_now_ms() < frame_deadline)
        {
            ...
            render_draw(game_get_state(game),
                        ...
                        highscore_count);
            if (has_3d)
                render_3d_draw(game_get_state(game),
                               game_config_get_player_name(cfg),
                               highscores,
                               highscore_count,
                               delta_s);
            /* Minimal sleep to yield CPU and prevent busy-wait, but allow high FPS */
            platform_sleep_ms(1);
        }
        tick++;
    }
clean_done:
    if (game_player_is_active(game, 0)
        && game_player_current_score(game, 0) > 0)
    {
        int final_score = game_player_current_score(game, 0);
        /* Only prompt for a name when the score would enter the high score
         * list. */
        HighScore** cur       = NULL;
        int         cur_cnt   = persist_read_scores(".snake_scores", &cur);
        bool        qualifies = false;
