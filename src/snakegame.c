...
if(!persist_append_score(".snake_scores", name_buf, death_score)) { console_warn("Failed to append score for %s\n", name_buf); }
...
if(!persist_append_score(".snake_scores", name_buf, final_score)) { console_warn("Failed to append final score for %s\n", name_buf); }
