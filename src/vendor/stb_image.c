...
load_bmp(const char* filename, int* x, int* y, int* channels_out)
{
    ...
    unsigned char* data =
        (unsigned char*)malloc((size_t)width * (size_t)height * 4);
    if (!data)
    {
        fclose(f);
        return NULL;
...
static unsigned char*
load_png(const char* filename, int* x, int* y, int* channels_out)
{
    FILE* f = fopen(filename, "rb");
    if (!f)
        return NULL;
    ...
    int            bit_depth = 0, color_type = 0, compression = 0, filter = 0,
        interlace            = 0;
    int            seen_IHDR = 0;
    ...
    {
        unsigned char* scan =
            decompressed + yrow * ((size_t)width * channels + 1);
        ...
        for (size_t i = 0; i < (size_t)width * channels; i++)
        {
            ...
            unsigned char up_left =
                (prev_row && i >= channels) ? prev_row[i - channels] : 0;
            ...
            switch (filter_type)
            {
                case 0:
                    ...
                    break;
                case 1:
                    res = (unsigned char)(val + left);
                    break;
                case 2:
                    res = (unsigned char)(val + up);
                    break;
                case 3:
                {
                    unsigned int avg =
                        ((unsigned int)left + (unsigned int)up) >> 1;
        ...
        for (size_t xpix = 0; xpix < (size_t)width; xpix++)
        {
            ...
            if (color_type == 3)
            {
                ...
                unsigned char a =
                    (trns && (size_t)idx < trns_len) ? trns[(size_t)idx] : 255;
                out[out_idx + 0] = r;
                out[out_idx + 1] = g;
                out[out_idx + 2] = b;
                out[out_idx + 3] = a;
