...
#include "stb_image.h"
/* Minimal implementation wrapper using the system libpng and libjpeg is not provided.
   To keep this compact, we'll implement a tiny loader for BMP only (sufficient for tests),
   and leave PNG support to stb_image if available on the system (otherwise loading PNG will fail).
...
static unsigned char* load_bmp(const char* filename, int* x, int* y, int* channels_out) {
    ...
    unsigned char* data = (unsigned char*)malloc((size_t)width * (size_t)height * 4);
    if(!data) { fclose(f); return NULL; }
    fseek(f, dataOffset, SEEK_SET);
    unsigned char* raw = (unsigned char*)malloc(dataSize);
    if(fread(raw, 1, dataSize, f) != dataSize) { free(raw); free(data); fclose(f); return NULL; }
    fclose(f);
...
static unsigned char* load_png(const char* filename, int* x, int* y, int* channels_out) {
    ...
    if(memcmp(sig, png_sig, 8) != 0) { fclose(f); return NULL; }
    ...
    while(1) {
        ...
        if(len > 0) {
            ...
            if(!data) { free(idat); fclose(f); return NULL; }
            if(fread(data, 1, len, f) != len) { free(data); free(idat); fclose(f); return NULL; }
        }
        unsigned char crc[4];
        if(fread(crc, 1, 4, f) != 4) { free(data); free(idat); fclose(f); return NULL; }

        if(strcmp(type, "IHDR") == 0) {
            if(len < 13) { free(data); free(idat); fclose(f); return NULL; }
            ...
            if(len == 0 || len % 3 != 0) { free(data); free(idat); fclose(f); return NULL; }
            plte = (unsigned char*)malloc(len);
            if(!plte) { free(data); free(idat); fclose(f); return NULL; }
    ...
    if(!seen_IHDR || idat_len == 0) { free(idat); fclose(f); return NULL; }
    if(bit_depth != 8 || compression != 0 || filter != 0 || interlace != 0) { free(idat); fclose(f); return NULL; }
    size_t channels;
    if(color_type == 6) channels = 4;
    else if(color_type == 2) channels = 3;
    else if(color_type == 3) channels = 1; /* palette-indexed */
    else channels = 0;
    if(channels == 0) { free(idat); fclose(f); return NULL; }

    size_t expected = (size_t)(1 + (size_t)width * channels) * (size_t)height;
    unsigned char* decompressed = (unsigned char*)malloc(expected);
    if(!decompressed) { free(idat); fclose(f); return NULL; }
    uLongf destLen = (uLongf)expected;
    int zret = uncompress(decompressed, &destLen, idat, (uLong)idat_len);
    free(idat);
    fclose(f);
    if(zret != Z_OK || destLen != expected) { free(decompressed); return NULL; }

    unsigned char* out = (unsigned char*)malloc((size_t)width * (size_t)height * 4);
    if(!out) { free(decompressed); return NULL; }
    unsigned char* prev_row = NULL;
    unsigned char* recon = (unsigned char*)malloc((size_t)width * channels);
    if(!recon) { free(decompressed); free(out); return NULL; }
    ...
    for(size_t yrow = 0; yrow < (size_t)height; yrow++) {
        ...
        for(size_t i = 0; i < (size_t)width * channels; i++) {
            ...
            switch(filter_type) {
                ...
                default: free(recon); free(decompressed); free(out); return NULL;
            }
            recon[i] = res;
        }
        for(size_t xpix = 0; xpix < (size_t)width; xpix++) {
            ...
            if(color_type == 3) {
                ...
                if((size_t)idx * 3 + 2 >= plte_len) { /* corrupt palette */
                    free(recon); free(decompressed); free(out); if(plte) free(plte); if(trns) free(trns); return NULL; }
        ...
        if(!prev_row) { free(recon); free(decompressed); free(out); return NULL; }
...
unsigned char *stbi_load(char const *filename, int *x, int *y, int *channels_in_file, int desired_channels) {
    ...
    if(fread(sig, 1, 8, f) == 8) {
        ...
        if(memcmp(sig, png_sig, 8) == 0) {
            return load_png(filename, x, y, channels_in_file);
        }
    } else {
        fclose(f);
    }
